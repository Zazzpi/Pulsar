...
## Для продажника: готовое приложение

Скачайте [Pulsar.exe для Windows](https://github.com/Zazzpi/Pulsar/releases/latest/download/Pulsar.exe)
и откройте двойным щелчком. Устанавливать Python, Docker или вводить команды
на компьютере продажника не нужно.

- **Показать тестовое:** нажмите «Посмотреть демо без сервера». Клиенты, сводки,
  вкладки, наблюдения и заметки доступны сразу на тестовых данных.
- **Работать с реальными данными:** введите адрес общего сервера Pulsar и
  учётные данные, полученные от администратора. Сервер разворачивается один
  раз для всего отдела; сам EXE не содержит backend и credentials WMS.

Windows 10/11, x64. Кэш, данные демо и журнал находятся в `%LOCALAPPDATA%\Pulsar`.
Подробности: [Windows-приложение](docs/WINDOWS.md).

**Linux:** скачайте [архив Pulsar для Linux x86_64](https://github.com/Zazzpi/Pulsar/releases/latest/download/Pulsar-linux-x86_64.tar.gz),
распакуйте и откройте `Pulsar-linux-x86_64` двойным щелчком. Python и Docker
не нужны. Сборка рассчитана на Ubuntu 24.04+/Debian 13 с графическим рабочим
столом. Windows EXE на Linux напрямую не запускается.

Для демонстрации: откройте демо, найдите «Ромашка», просмотрите вкладки,
включите наблюдение и сохраните заметку. В «Мои клиенты» появится наблюдение;
заметка останется после перезапуска.
...
## Сервер: запуск для администратора или разработчика

Следующие команды относятся к подготовке общего сервера и разработке.
Продажник запускает готовый EXE из раздела выше. Для серверного DEMO нужны
Git, Docker с Compose и Python для скрипта подготовки.
...
Разработчик может запускать desktop из исходников:
...
- Windows desktop распространяется как один EXE; отдельный установщик не требуется.
  Для Linux/macOS остаётся запуск из исходников.
